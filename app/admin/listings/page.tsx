...
import { useProperties, useAddFeaturedProperty, useRemoveFeaturedProperty } from "@/services/property/PropertyQueries";
...
function ListingsContent() {
    ...
    return (
      <div className="flex flex-col items-center gap-3 mt-8">
        <div className="flex items-center gap-1.5">
          ...
          {startPage > 1 && (
            <>
              <button onClick={() => handlePageChange(1)} disabled={isLoading} className={`w-8 h-8 text-sm rounded-lg border transition ${1 === currentPageNum ? "bg-emerald-500 text-white border-emerald-500" : "border-slate-200 text-slate-600 hover:bg-white"}`}>1</button>
          ...
          {endPage < totalPages && (
            <>
              ...
              <button onClick={() => handlePageChange(totalPages)} disabled={isLoading} className={`w-8 h-8 text-sm rounded-lg border transition ${totalPages === currentPageNum ? "bg-emerald-500 text-white border-emerald-500" : "border-slate-200 text-slate-600 hover:bg-white"}`}>{totalPages}</button>
            </>
          )}
          <button
            onClick={() => handlePageChange(currentPageNum + 1)}
  ...
  return (
    <div className="px-6 lg:px-8 max-w-[1280px] mx-auto">
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-5 animate-in fade-in slide-in-from-top-2">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
            <div>
              <label className="text-xs font-medium text-slate-500 mb-1 block">Min Beds</label>
              <input type="number" min="0" value={filters.bed_min || ""} onChange={(e) => setFilters({ ...filters, bed_min: e.target.value })} className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="0" />
            </div>
            <div>
              <label className="text-xs font-medium text-slate-500 mb-1 block">Max Beds</label>
              <input type="number" min="0" value={filters.bed_max || ""} onChange={(e) => setFilters({ ...filters, bed_max: e.target.value })} className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="Any" />
            </div>
            <div>
              <label className="text-xs font-medium text-slate-500 mb-1 block">Min Baths</label>
              <input type="number" min="0" value={filters.bath_min || ""} onChange={(e) => setFilters({ ...filters, bath_min: e.target.value })} className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="0" />
            </div>
            <div>
              <label className="text-xs font-medium text-slate-500 mb-1 block">Max Baths</label>
              <input type="number" min="0" value={filters.bath_max || ""} onChange={(e) => setFilters({ ...filters, bath_max: e.target.value })} className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="Any" />
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={handleSearch} disabled={isLoading} className="px-5 py-2 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium transition-colors">
              Apply Filters
            </button>
            <button onClick={handleClearFilters} className="px-5 py-2 rounded-full border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 transition-colors">
              Clear All
            </button>
          </div>
        </div>
      ...
      {featuredSuccess && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-xl mb-5 text-sm" role="alert">
          {featuredSuccess}
        </div>
      )}
      {featuredError && (
        ...
        {Array.isArray(listings) && listings.length > 0 ? (
          listings.map((listing: any) => {
            ...
            return (
              <div
                key={listing.id}
                className="bg-white rounded-2xl border border-slate-100 hover:shadow-lg transition-shadow flex overflow-hidden group cursor-pointer"
                onClick={() => router.push(`/admin/listings/${listing.id}?from_page=${currentPage}`)}
              >
                {/* Image */}
                <div className="relative w-[200px] min-h-[160px] flex-shrink-0 overflow-hidden">
                  <img
                    src={getListingImage(listing)}
                    alt={listing.title || listing.name || listing.address || `Listing ${listing.id}`}
                ...
                <div className="flex-1 p-5 flex items-center justify-between min-w-0">
                  <div className="min-w-0 flex-1 pr-6">
                    ...
                    <div className="flex items-center gap-2.5 mb-1">
                      ...
                      {listing.status && (
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${statusBadge(listing.status)}`}>
                          {listing.status}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-5 text-xs text-slate-500">
                      {listing.beds !== undefined && listing.beds !== null && (
                        <span className="flex items-center gap-1.5">
                          ...
                          <span><span className="font-medium text-slate-700">{listing.beds}</span> {listing.beds === 1 ? "Bed" : "Beds"}</span>
                        </span>
                      )}
                      {listing.baths !== undefined && listing.baths !== null && (
                        <span className="flex items-center gap-1.5">
                          <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                          <span><span className="font-medium text-slate-700">{listing.baths}</span> Baths</span>
                        </span>
                      )}
                      {sqft && (
                        <span className="flex items-center gap-1.5">
                          ...
                          <span><span className="font-medium text-slate-700">{sqft}</span> Sqft</span>
                        </span>
                      )}
                      {views !== null && (
                        <span className="flex items-center gap-1.5">
                          <EyeIcon className="w-4 h-4 text-slate-400" />
                          <span><span className="font-medium text-slate-700">{views}</span> Views</span>
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                    ...
                    <Link
                      ...
                      className="px-4 py-2 text-sm font-medium rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50 transition"
                    >
                      View
                    </Link>
                    <button
                      onClick={() => handleToggleFeatured(listing.id, listing.is_featured == 1)}
                      disabled={featuringListingId === listing.id || isLoading}
