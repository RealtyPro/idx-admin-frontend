...
export default function PropertyDetailsPage() {
  ...
  const getImageUrl = () => {
    ...
    if (!img) return null;
    if (typeof img === "string") return img.startsWith("http") ? img : `https://demorealestate2.webnapps.net/storage/${img}`;
    if (typeof img === "object" && !Array.isArray(img) && img.path) return img.path.startsWith("http") ? img.path : `https://demorealestate2.webnapps.net/storage/${img.path}`;
    if (Array.isArray(img) && img.length > 0) { const f = img[0]; return typeof f === "string" ? (f.startsWith("http") ? f : `https://demorealestate2.webnapps.net/storage/${f}`) : f?.path ? (f.path.startsWith("http") ? f.path : `https://demorealestate2.webnapps.net/storage/${f.path}`) : null; }
    return null;
  };
