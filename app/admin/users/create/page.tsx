...
export default function CreateUserPage() {
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await axiosInstance.post("v1/user/customer", null, { params });
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 lg:p-8">
        <form className="space-y-6" onSubmit={onSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {/* Attach Listing */}
            <div ref={dropdownRef}>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Attach Listing</label>
              {!selectedProperty ? (
                <div className="relative">
                  ...
                  {showDropdown && (
                    <div className="absolute top-full left-0 right-0 z-20 mt-1 rounded-xl border border-slate-200 bg-white shadow-lg max-h-60 overflow-auto">
                      {propertyLoading && <p className="px-4 py-3 text-sm text-slate-400">Loading listings...</p>}
                      ...
                      {!propertyLoading &&
                        propertyOptions.map((item) => (
                          ...
                          >
                            <p className="font-medium text-slate-800 truncate">{propertyLabel(item)}</p>
                            {item.city && <p className="text-xs text-slate-500 truncate">{item.city}</p>}
                          </button>
                        ))}
                    </div>
                  )}
                ...
                <div className="flex items-center justify-between gap-3 px-3 py-2 rounded-xl border border-emerald-200 bg-emerald-50">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-emerald-700 truncate">{propertyLabel(selectedProperty)}</p>
                    {selectedProperty.city && <p className="text-xs text-emerald-600 truncate">{selectedProperty.city}</p>}
                  </div>
                  <button
                    type="button"
                    onClick={() => {
